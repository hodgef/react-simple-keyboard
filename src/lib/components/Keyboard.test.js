...
it('Keyboard renders without crashing', () => {
  ...
  ReactDOM.render(<Keyboard baseClass={"simple-keyboard"} ref={r => keyboardInstance = r} {...keyboardOptions} />, div);
});

it('Keyboard standard buttons are working', () => {
  testLayoutStdButtons();

  keyboardInstance.keyboard.setOptions({
    layoutName: "shift",
    maxLength: 42
  });
...
function testLayoutStdButtons(){
  ...
  Array.from(rows).forEach(row => {
    Array.from(row.children).forEach((button) => {
      ...
      fullInput = keyboardInstance.keyboard.utilities.getUpdatedInput(label, fullInput, keyboardInstance.keyboard.options, null);
  ...
  if(stdBtnCount !== fullInput.length)
    ...
    console.log("STANDARD_BUTTONS PASSED:", keyboardInstance.keyboard.options.layoutName, stdBtnCount, fullInput.length);
}